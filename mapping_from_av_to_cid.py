...
# 基础目录（存放搜索结果的地方）
base_dir = r"D:\Bilibili搜索结果" # 修改为你的搜索结果目录

# 输出映射文件路径
output_file = os.path.join(base_dir, "视频CID映射.json")

def extract_video_id(arcurl):
    """从arcurl中提取AV号或BV号"""
...
def get_video_cid(video_id_dict):
    """使用AV号或BV号获取CID"""
    ...
    try:
        ...
        if result['code'] == 0:
            ...
            return {'main_cid': main_cid, 'title': result['data'].get('title', ''), 'parts': all_parts if all_parts else None}
        else:
...
def process_search_results():
    ...
    # 计算需要处理的文件总数
    for keyword_dir in os.listdir(base_dir):
        keyword_path = os.path.join(base_dir, keyword_dir)
                            ...
                            try:
                                ...
                                if 'data' in search_data and 'result' in search_data['data']:
                                    ...
                                    for video in video_results:
                                        ...
                                        total_videos += 1
                                        video_id_dict = extract_video_id(arcurl)
                                        
                                        if video_id_dict:
                                            sleep_time = random_sleep()
                                            cid_info = get_video_cid(video_id_dict)
